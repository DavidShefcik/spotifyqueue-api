            ...
            userModel.findOne({"token": token}, (error, user) => {
                if(error) {
                    ...
                    if(user) {
                        ...
                        axios.post("https://accounts.spotify.com/api/token", qs.stringify(data), {headers: headers}).then(res => {
                            accessToken = res["data"]["access_token"];
                            userModel.updateOne({"token": token}, {$set: {"access_token": accessToken}}, {new: true}, (error, doc) => {
                                if(error) {
                                    if(process.env.PRODUCTION === "false") {
                                        console.log(error);
                                    }
